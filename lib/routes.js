...
function MessageHandler(db) {
	...
	this.postNewMessage = function(req, res) {
		...
		fs.rename(req.files.message.path, upload_path, function (err) {
			...
			var webPath = 'store/' + path.basename(req.files.message.path);
			webPath = webPath.split(".")[0] + ".wav";
			console.log(JSON.stringify(req.google_user));
			var message = { 'audio': webPath, 
			...
			var wavPath = upload_path.split(".")[0] + ".wav";
			var command = "ffmpeg -i " + upload_path + " -ab 128 " + wavPath;
